...
public class AppMap {
	...
	public static void main(String[] args) {
		...
		Pais aux = new Pais(34, "Espanha", 600);
		dic2.put(41,aux);
		dic2.put(42,aux);
		System.out.println("\nConteúdo do dic2 após inserir a Espanha:");
		for (int k : dic2.keySet())
			System.out.printf("K: %d  V: %s\n", k, dic2.get(k));
			
		aux.setCodigo(43);
		System.out.println("\nConteúdo do dic2 após modificar código da espanha:");
		for (int k : dic2.keySet())
			System.out.printf("K: %d  V: %s\n", k, dic2.get(k));
		
		dic2.remove(42);
		System.out.println("\nConteúdo do dic2 após remover a chave 42:");
		for (int k : dic2.keySet())
			System.out.printf("K: %d  V: %s\n", k, dic2.get(k));
		...
		try (BufferedReader reader = Files.newBufferedReader(path1, Charset.forName("utf8"))) {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (rotas.containsKey(origem)) {
					HashSet<String> aux1 = rotas.get(origem);
					aux1.add(destino);
					// precisa?
					rotas.put(origem, aux1);

					// para os grandinhos (em termos de programação...)
					// (rotas.get(origem)).add(destino);
				}
				else {
					HashSet<String> aux2 = new HashSet<>();
					aux2.add(destino);
					rotas.put(origem, aux2);
				}
			}
		}
		catch (IOException x) {
